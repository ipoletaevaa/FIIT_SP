...
void *allocator_buddies_system::get_buddy(void *block) noexcept
{
    ...
    size_t block_size = __detail::power_size(target_k);
    uintptr_t ptr = reinterpret_cast<std::byte*>(block) - reinterpret_cast<std::byte*>(get_blocks_begin_ptr(_trusted_memory));
    
    size_t total_size = __detail::power_size(get_header(_trusted_memory)->k);
    
    uintptr_t buddy_offset = ptr ^ (1ULL << target_k);
    
    if (buddy_offset >= total_size)
    {
        return nullptr;
    }
    
    return reinterpret_cast<std::byte*>(get_blocks_begin_ptr(_trusted_memory)) + buddy_offset;
}
...
void allocator_buddies_system::do_deallocate_sm(void *at)
{
    ...
    void *buddy_ptr;
    size_t max_k = header->k;

    while (curr->size < max_k) {
        buddy_ptr = get_buddy(curr);
        
        if (buddy_ptr == nullptr) break;
