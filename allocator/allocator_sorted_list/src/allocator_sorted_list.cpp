// allocator_sorted_list.cpp
#include "../include/allocator_sorted_list.h"
#include <cstring>
#include <new>

allocator_sorted_list::allocator_header* allocator_sorted_list::get_header() const
{
    return reinterpret_cast<allocator_header*>(_trusted_memory);
}

void* allocator_sorted_list::get_blocks_begin_ptr() const
{
        ...
        allocator_with_fit_mode::fit_mode allocate_fit_mode)
{
    ...
    first_fr->block_size = space_size - block_metadata_size;
...
void allocator_sorted_list::do_deallocate_sm(void *at)
{
    ...
    void* block = static_cast<std::byte*>(at) - block_metadata_size;
    void* start = get_blocks_begin_ptr();
    void* mem_end = reinterpret_cast<std::byte*>(_trusted_memory) + header->total_size;
    
    if (block < start || block >= mem_end)
        throw std::logic_error("pointer out of bounds");

    bool is_occupied = false;
    for (auto it = begin(); it != end(); ++it) {
        if (it.occupied() && *it == block) {
            is_occupied = true;
            break;
        }
    }
    if (!is_occupied)
        throw std::logic_error("pointer does not point to an occupied block");

    block_header *curr = reinterpret_cast<block_header*>(block);
